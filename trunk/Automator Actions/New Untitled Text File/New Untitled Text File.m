...
- (id)runWithInput:(id)input fromAction:(AMAction *)anAction error:(NSDictionary **)errorInfo {
	...
	// The key initialContentsOption is in the Parameters NSObjectController instantiated in the Nib
	// and then bound (via Cocoa bindings) to the NSPopUpButton's selected index.
	...
	// Use the action's input as the location to create the new file. If it's not set, use ~/Desktop/.
	NSString *path = nil;
	...
	// Set the file name to Untitled.txt. If it already exists at the path, append an index number.
	...
	for (index; [[NSFileManager defaultManager] fileExistsAtPath:newFile]; index++)
		newFile = [path stringByAppendingPathComponent:[NSString stringWithFormat:@"Untitled %d.txt", index]];
